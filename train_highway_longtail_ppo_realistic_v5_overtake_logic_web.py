...
This V5 overtaking-logic version keeps the active overtaking behaviour from V2/V3,
but adds a stronger front-vehicle speed governor. This directly addresses the
video failure mode where the ego car enters the overtaking lane, keeps a high
target speed from previous FASTER actions, and rear-ends a slower vehicle ahead.
...
6. Add a speed governor:
   - even if the policy outputs IDLE, the wrapper caps ego.target_speed when a
     front vehicle is too close
   - the safety check looks at both the current lane and the target lane during
     a lane change, so entering the fast lane cannot ignore a car already ahead

Recommended commands:
    # Train the safer but active discrete version
    python train_highway_longtail_ppo_realistic_v5_overtake_logic_web.py --timesteps 300000

    # Evaluate and record videos
    python train_highway_longtail_ppo_realistic_v5_overtake_logic_web.py --skip-train --video-dir videos --debug-eval

    # Make traffic harder/more complex
    python train_highway_longtail_ppo_realistic_v5_overtake_logic_web.py --traffic-mode dense --timesteps 400000

    # Experimental continuous steering/throttle; this needs a fresh model
    python train_highway_longtail_ppo_realistic_v5_overtake_logic_web.py --continuous-action --timesteps 500000
"""
...
class RealisticHighwayWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        fast_lane_safety_multiplier: float = 1.75,
        fast_lane_min_ttc: float = 4.0,
        fast_lane_accel_gap_multiplier: float = 1.60,
        enable_speed_governor: bool = True,
        target_lane_safety_check: bool = True,
        speed_governor_gap_multiplier: float = 1.90,
        fast_lane_speed_margin: float = 0.0,
        normal_lane_speed_margin: float = 1.20,
        useful_lane_change_only: bool = True,
        lane_change_min_benefit: float = 22.0,
        return_right_clear_time: float = 10.0,
    ):
        super().__init__(env)
        ...
        self.enable_speed_governor = bool(enable_speed_governor)
        self.target_lane_safety_check = bool(target_lane_safety_check)
        self.speed_governor_gap_multiplier = float(speed_governor_gap_multiplier)
        self.fast_lane_speed_margin = float(fast_lane_speed_margin)
        self.normal_lane_speed_margin = float(normal_lane_speed_margin)
        self.useful_lane_change_only = bool(useful_lane_change_only)
        self.lane_change_min_benefit = float(lane_change_min_benefit)
        self.return_right_clear_time = float(return_right_clear_time)
        ...
        self.speed_governor_reason = ""
        self.speed_governor_target_speed: Optional[float] = None
    ...
    def reset(self, **kwargs: Any):
        ...
        self.speed_governor_reason = ""
        self.speed_governor_target_speed = None
    ...
    def step(self, action: Any):
        ...
        filtered_action = self._apply_speed_governor_before_step(filtered_action)
    ...
    def _apply_overtake_assist(self, action: int) -> int:
        ...
        critical, caution, reason = self._relevant_front_safety_state(ego)
        ...
        # Start overtaking only when there is a real reason AND the left lane
        # gives a clear benefit. This avoids meaningless zig-zag lane changes.
        if self._overtake_opportunity(ego):
            if self._left_lane_good_for_overtake(ego):
                if action in (IDLE, FASTER, SLOWER):
                    self.assist_reason = "start_overtake_left_clear_lane"
                    return LANE_LEFT
            elif self._following_too_close(ego) and action != SLOWER:
                self.assist_reason = "too_close_no_useful_left_gap_slow_down"
                return SLOWER
    ...
    def _apply_discrete_lane_rules(self, action: int) -> int:
        ...
        if action in (LANE_LEFT, LANE_RIGHT):
            ...
            if self.useful_lane_change_only and not self._lane_change_has_clear_purpose(ego, action):
                self.lane_change_block_reason = "no_clear_lane_change_purpose"
                return IDLE
    ...
    def _apply_speed_governor_before_step(self, action: Any) -> Any:
        """Cap ego.target_speed before env.step when a front vehicle is too close.

        This is the important V4/V5 fix. In DiscreteMetaAction, IDLE does not mean
        braking; if the vehicle already has a high target_speed from earlier
        FASTER actions, it may keep accelerating into the front car. The governor
        directly lowers target_speed when the current lane or target lane is not
        clear enough.
        """
        if not self.enable_speed_governor or self.is_continuous_action:
            return action

        ego = self.env.unwrapped.vehicle
        if ego is None or not hasattr(ego, "target_speed"):
            return action

        best_cap: Optional[float] = None
        best_reason = ""
        strongest_brake = False

        for lane_id in self._safety_lane_ids_for_ego(ego):
            ...
            gap_ratio = front_gap / max(safe_distance, 1e-6)
            ttc_risky = ttc is not None and ttc < min_ttc + (1.2 if lane_id == 0 else 0.8)
            gap_risky = front_gap < self.speed_governor_gap_multiplier * safe_distance

            if not gap_risky and not ttc_risky:
                continue

            # In the fast/overtaking lane, do not allow target_speed to sit much
            # above the front vehicle's speed unless the gap is very large.
            speed_margin = self.fast_lane_speed_margin if lane_id == 0 else self.normal_lane_speed_margin

            if gap_ratio < 0.75 or (ttc is not None and ttc < min_ttc):
                # Critical: aim below the front vehicle speed to create space.
                cap = max(5.0, front_speed - 1.2 - 0.8 * self.weather_intensity)
                strongest_brake = True
                reason = f"critical_speed_cap_lane_{lane_id}"
            elif gap_ratio < 1.10 or ttc_risky:
                # Unsafe/caution: match the front vehicle speed, with almost no
                # positive margin in the fast lane.
                cap = max(6.0, front_speed + speed_margin)
                reason = f"caution_speed_cap_lane_{lane_id}"
            else:
                # Not enough space for acceleration; allow only a gentle close.
                cap = max(7.0, front_speed + speed_margin + 0.8)
                reason = f"soft_speed_cap_lane_{lane_id}"
        ...
        if best_cap is not None:
            old_target = float(getattr(ego, "target_speed", best_cap))
            ego.target_speed = min(old_target, float(best_cap))
            self.speed_governor_target_speed = float(ego.target_speed)
            self.speed_governor_reason = best_reason
            if self.safety_shield_reason:
                self.safety_shield_reason += f"+{best_reason}"
            else:
                self.safety_shield_reason = best_reason

            # If the current action is still passive while the cap is critical,
            # make the discrete controller actively request slower behaviour too.
            if strongest_brake and int(action) in (IDLE, FASTER):
                return SLOWER
            if int(action) == FASTER:
                return IDLE
    ...
    def _safety_lane_ids_for_ego(self, ego: Any) -> list[int]:
        """Lane IDs whose front vehicles must be considered for rear-end safety.

        During a lane change highway-env may still report ego.lane_index as the
        old lane while ego.target_lane_index is already the destination lane.
        V3 sometimes checked only the old lane, so a vehicle already ahead in the
        fast lane could be missed until it was too late. V4 checks both lanes.
        """
        lane_ids: list[int] = []
        current_lane_id = self._lane_id(ego)
        if current_lane_id is not None:
            lane_ids.append(current_lane_id)

        if self.target_lane_safety_check:
            target_lane_index = getattr(ego, "target_lane_index", None)
            if target_lane_index is not None:
                try:
                    target_lane_id = int(target_lane_index[2])
                    if target_lane_id not in lane_ids:
                        lane_ids.append(target_lane_id)
                except Exception:
                    pass

        return lane_ids

    def _relevant_front_safety_state(self, ego: Any) -> tuple[bool, bool, str]:
        """Worst front-risk state across current and target lanes."""
        best_state = (False, False, "no_front")
        best_rank = -1
        best_gap: Optional[float] = None
        best_ttc: Optional[float] = None
        best_safe_distance: Optional[float] = None

        for lane_id in self._safety_lane_ids_for_ego(ego):
            critical, caution, reason = self._front_safety_state(ego, lane_id)
            rank = 2 if critical else (1 if caution else 0)

            # Prefer the more severe state. If severity is equal, prefer the
            # smaller relative gap because it is the more urgent risk.
            rel_gap = None
            if self.current_front_gap is not None and self.current_lane_safe_distance:
                rel_gap = self.current_front_gap / max(self.current_lane_safe_distance, 1e-6)

            should_take = rank > best_rank
            if rank == best_rank and rel_gap is not None and best_gap is not None and self.current_lane_safe_distance:
                should_take = rel_gap < best_gap / max(best_safe_distance or 1e-6, 1e-6)

            if should_take:
                best_state = (critical, caution, f"{reason}_lane_{lane_id}")
                best_rank = rank
                best_gap = self.current_front_gap
                best_ttc = self.current_front_ttc
                best_safe_distance = self.current_lane_safe_distance

        self.current_front_gap = best_gap
        self.current_front_ttc = best_ttc
        self.current_lane_safe_distance = best_safe_distance
        return best_state
    ...
    def _lane_change_has_clear_purpose(self, ego: Any, action: int) -> bool:
        """Block policy-created lane changes that do not have a driving purpose.

        The car should not weave just because the policy occasionally outputs
        LANE_LEFT/LANE_RIGHT.  A left change is useful when the current lane is
        blocked by a slower front vehicle and the target-left lane is clearly
        better.  A right change is useful when it is a safe return/right-keep
        move and the target lane will not immediately force another overtake.
        """
        lane_id = self._lane_id(ego)
        if lane_id is None:
            return False
        ...
        if action == LANE_LEFT:
            return self._left_lane_good_for_overtake(ego)

        if action == LANE_RIGHT:
            # Leaving the fast lane is realistic only if the right lane is usable;
            # otherwise the car would return right and immediately weave left again.
            if lane_id == 0:
                return self._right_lane_usable_after_return(ego)
            # For normal lanes, right-keep is okay only when there is no immediate
            # slow vehicle blocking the target lane.
            return self._right_lane_usable_after_return(ego) and not self._overtake_opportunity(ego)

        return True

    def _left_lane_good_for_overtake(self, ego: Any) -> bool:
        """True if moving one lane left is both safe and meaningfully useful.

        This implements the behaviour you described: if the ego lane has a
        slower vehicle ahead and the fast/left lane is open, use it to overtake.
        If the left lane also has a close vehicle ahead, do not weave left;
        slow down and follow safely instead.
        """
        lane_id = self._lane_id(ego)
        ...
        if not self._overtake_opportunity(ego):
            return False
        if not self._can_change_lane(ego, LANE_LEFT):
            return False

        current_front, current_gap = self._front_vehicle_and_gap(ego)
        if current_front is None or current_gap is None:
            return False

        ego_speed = float(getattr(ego, "speed", 0.0))
        current_front_speed = float(getattr(current_front, "speed", 0.0))
        target_front, target_gap = self._closest_front_in_lane(ego, target_lane_id)

        # Empty target lane ahead: this is the ideal overtake case.
        if target_front is None or target_gap is None:
            return True

        target_front_speed = float(getattr(target_front, "speed", 0.0))
        target_safe = self._lane_safe_following_distance(ego_speed, target_lane_id)

        # The target lane must provide clearly more usable space than the current
        # blocked lane.  This avoids lane changes that only move the car behind
        # another nearby vehicle.
        required_target_gap = max(
            1.45 * target_safe,
            current_gap + self.lane_change_min_benefit,
            65.0 if target_lane_id == 0 else 50.0,
        )

        target_lane_has_space = target_gap >= required_target_gap
        target_lane_is_faster = target_front_speed >= current_front_speed + 1.0

        return bool(target_lane_has_space and target_lane_is_faster)

    def _right_lane_usable_after_return(self, ego: Any) -> bool:
        """True if moving right will not cause immediate weaving back left."""
        lane_id = self._lane_id(ego)
        max_lane_id = self._max_lane_id(ego)
        if lane_id is None or max_lane_id is None or lane_id >= max_lane_id:
            return False
        target_lane_id = lane_id + 1

        if not self._can_change_lane(ego, LANE_RIGHT):
            return False
        ...
        ego_speed = float(getattr(ego, "speed", 0.0))
        front_speed = float(getattr(target_front, "speed", 0.0))
        target_safe = self._lane_safe_following_distance(ego_speed, target_lane_id)

        # Basic following gap must be comfortable.
        if target_gap < 1.20 * target_safe:
            return False

        # If we would catch the target-lane front vehicle soon, returning right
        # would only create the left-right-left oscillation seen in videos.
        closing_speed = ego_speed - front_speed
        if closing_speed > 0.3:
            time_until_blocked = target_gap / closing_speed
            if time_until_blocked < self.return_right_clear_time:
                return False
    ...
    def _should_leave_fast_lane(self, ego: Any) -> bool:
        ...
        if self.fast_lane_time < self.fast_lane_grace_sec:
            return False
        # Do not return right if that lane will be immediately blocked; otherwise
        # the vehicle weaves right and then left again after the cooldown.
        return self._right_lane_usable_after_return(ego)
    ...
    def _common_info(self, effective_action: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "speed_governor_reason": self.speed_governor_reason,
            "speed_governor_target_speed": self.speed_governor_target_speed,
            ...
            "left_lane_good_for_overtake": bool(self._left_lane_good_for_overtake(ego)) if ego is not None else False,
            "right_lane_usable_after_return": bool(self._right_lane_usable_after_return(ego)) if ego is not None else False,
...
def build_env(
    ...
    env = RealisticHighwayWrapper(
        ...
        enable_speed_governor=enable_speed_governor,
        target_lane_safety_check=target_lane_safety_check,
        speed_governor_gap_multiplier=speed_governor_gap_multiplier,
        fast_lane_speed_margin=fast_lane_speed_margin,
        normal_lane_speed_margin=normal_lane_speed_margin,
        useful_lane_change_only=useful_lane_change_only,
        lane_change_min_benefit=lane_change_min_benefit,
        return_right_clear_time=return_right_clear_time,
    )
    env = Monitor(env)
    return env
...
def make_train_env(seed: int, args: argparse.Namespace):
    return build_env(
        ...
        enable_speed_governor=not args.no_speed_governor,
        target_lane_safety_check=not args.no_target_lane_safety_check,
        speed_governor_gap_multiplier=args.speed_governor_gap_multiplier,
        fast_lane_speed_margin=args.fast_lane_speed_margin,
        normal_lane_speed_margin=args.normal_lane_speed_margin,
        useful_lane_change_only=not args.allow_nonpurpose_lane_changes,
        lane_change_min_benefit=args.lane_change_min_benefit,
        return_right_clear_time=args.return_right_clear_time,
    )
...
def model_stem(args: argparse.Namespace) -> str:
    ...
    return f"ppo_highway_longtail_realistic_v5_overtake_logic_{suffix}_{args.traffic_mode}"
...
def evaluate(model_path: Path, args: argparse.Namespace) -> None:
    ...
    for episode in range(args.eval_episodes):
        ...
        speed_governor_counter: Counter[str] = Counter()
        ...
        while not (terminated or truncated):
            ...
            if args.debug_eval:
                ...
                speed_governor_reason = str(info.get("speed_governor_reason", ""))
                ...
                if speed_governor_reason:
                    speed_governor_counter[speed_governor_reason] += 1
                if block_reason:
        ...
        if args.debug_eval:
            ...
            print(f"  speed_governor={dict(speed_governor_counter)}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--log-dir", type=str, default="runs/ppo_highway_longtail_realistic_v5_overtake_logic")
    ...
    parser.add_argument("--fast-lane-safety-multiplier", type=float, default=1.75,
                        help="Multiplier for following distance in the left-most overtaking lane.")
    parser.add_argument("--fast-lane-min-ttc", type=float, default=4.0,
                        help="Minimum time-to-collision in seconds before the fast-lane safety shield brakes.")
    parser.add_argument("--fast-lane-accel-gap-multiplier", type=float, default=1.60,
                        help="FASTER is blocked in the fast lane unless the front gap exceeds this multiplier times the fast-lane safe distance.")
    parser.add_argument("--no-speed-governor", action="store_true",
                        help="Disable the V4 target-speed governor. Usually keep this off.")
    parser.add_argument("--no-target-lane-safety-check", action="store_true",
                        help="Disable checking the target lane front vehicle during lane changes. Usually keep this off.")
    parser.add_argument("--speed-governor-gap-multiplier", type=float, default=1.90,
                        help="Begin capping target_speed when front gap is below this multiplier times safe distance.")
    parser.add_argument("--fast-lane-speed-margin", type=float, default=0.0,
                        help="Allowed speed margin above the front vehicle in the fast lane when gap is not clear, in m/s.")
    parser.add_argument("--normal-lane-speed-margin", type=float, default=1.20,
                        help="Allowed speed margin above the front vehicle in normal lanes when gap is not clear, in m/s.")
    parser.add_argument("--allow-nonpurpose-lane-changes", action="store_true",
                        help="Allow PPO-requested lane changes even when they have no clear overtaking/return-right purpose. Usually keep this off.")
    parser.add_argument("--lane-change-min-benefit", type=float, default=22.0,
                        help="Minimum extra front gap in the target lane before a left overtake lane-change is considered useful, in meters.")
    parser.add_argument("--return-right-clear-time", type=float, default=10.0,
                        help="Do not return right if the right lane will be blocked again within this many seconds.")
