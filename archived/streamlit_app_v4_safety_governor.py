#!/usr/bin/env python3
"""Streamlit dashboard for the V4 safety-governor highway PPO script.
...
DEFAULT_BACKEND = APP_DIR / "train_highway_longtail_ppo_realistic_v4_safety_governor_web.py"
DEFAULT_MODEL_DIR = APP_DIR / "models"
DEFAULT_LOG_DIR = APP_DIR / "runs" / "ppo_highway_longtail_realistic_v4_safety_governor"
...
def main() -> None:
    st.set_page_config(page_title="Highway PPO V4 Safety Governor Dashboard", layout="wide")
    st.title("Highway PPO V4 Safety Governor 参数控制台")
    st.caption("用 Streamlit 调参、训练、evaluation 录制，以及集中查看生成的视频。V4 加入 speed governor，重点减少超车道追尾。")

    with st.sidebar:
        st.header("路径设置")
        backend_path = to_abs_path(st.text_input("V4 backend script", value=str(DEFAULT_BACKEND)))
    ...
    with tab_train:
        st.markdown("这里会启动一次训练；训练完成后，如果没有勾选 `skip eval`，会自动按 eval episodes 录制视频。V4 默认开启 speed governor。")
