...
This V2 version fixes the common problem where the car becomes too conservative:
it no longer crashes, but it also almost never changes lane or overtakes.
...
Recommended commands:
    ...
    python train_highway_longtail_ppo_realistic_v2.py --timesteps 300000

    # Evaluate and record videos
    python train_highway_longtail_ppo_realistic_v2.py --skip-train --video-dir videos --debug-eval

    # Make traffic harder/more complex
    python train_highway_longtail_ppo_realistic_v2.py --traffic-mode dense --timesteps 400000

    # Experimental continuous steering/throttle; this needs a fresh model
    python train_highway_longtail_ppo_realistic_v2.py --continuous-action --timesteps 500000
"""
...
class RealisticHighwayWrapper(gym.Wrapper):
    ...
    def _apply_overtake_assist(self, action: int) -> int:
        ...
        if action == IDLE and not self._following_too_close(ego):
            ...
            if speed < self._target_cruise_speed() - 3.0:
    ...
    def _compute_reward(self, action: Any) -> float:
        ...
        if front_vehicle is not None and front_gap is not None:
            safe_distance = self._safe_following_distance(speed)
            ...
            if closing_speed > 0.2:
                ...
                min_ttc = 1.6 + 1.8 * self.weather_intensity
                if ttc < min_ttc:
                    reward -= 2.8 * (1.0 - ttc / min_ttc) * (1.0 + self.weather_intensity)
    ...
    def _lane_gap_safe(self, ego: Any, target_lane_id: int) -> bool:
        ...
        front_min_gap = max(10.0, 0.50 * self._safe_following_distance(ego_speed))
        rear_min_gap = 8.0 + 8.0 * self.weather_intensity
        ...
        for other in road.vehicles:
            ...
            if delta >= 0.0 and bumper_gap < front_min_gap:
                return False
            if delta < 0.0 and bumper_gap < rear_min_gap:
                return False
    ...
    def _following_too_close(self, ego: Any) -> bool:
        ...
        if front_gap is None:
            return False
        speed = float(getattr(ego, "speed", 0.0))
        return front_gap < 0.85 * self._safe_following_distance(speed)
    ...
    def _overtake_opportunity(self, ego: Any) -> bool:
        ...
        front_speed = float(getattr(front_vehicle, "speed", 0.0))
        target_speed = self._target_cruise_speed()
        safe_distance = self._safe_following_distance(ego_speed)
    ...
    def _common_info(self, effective_action: Any) -> dict[str, Any]:
        ...
        safe_distance = self._safe_following_distance(float(getattr(ego, "speed", 0.0))) if ego is not None else None
...
def model_stem(args: argparse.Namespace) -> str:
    ...
    return f"ppo_highway_longtail_realistic_v2_{suffix}_{args.traffic_mode}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="PPO on highway-v0 with active realistic overtaking, rain distance and cooldown rules."
    )

    parser.add_argument("--timesteps", type=int, default=300_000)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--device", type=str, default="auto")
    parser.add_argument("--log-dir", type=str, default="runs/ppo_highway_longtail_realistic_v2")
...
def main() -> None:
    ...
    if not args.skip_train:
        ...
        raise FileNotFoundError(
            ...
            "Train first, or make sure --continuous-action and --traffic-mode match the model type."
        )
